...
def data_load(args):
    ...
    ltarget_set = utils.ObjectImage_mul('', args.lt_dset_path, [train_transform, train_transform])
...
def train(args):
    ...
    if args.pl.startswith('atdoc_na'):
        ...
        mem_cls = torch.ones(len(dset_loaders["target"].dataset) + len(dset_loaders["ltarget"].dataset), args.class_num).cuda() / args.class_num
    ...
    for iter_num in range(1, args.max_iter + 1):
        ...
        targets_lt = torch.zeros(args.batch_size//3, args.class_num).scatter_(1, labels_ltarget.view(-1,1), 1)
        ...
        ####################################################################
        all_inputs = torch.cat([inputs_s, inputs_lt, inputs_t, inputs_lt2, inputs_t2], dim=0)
        all_targets = torch.cat([targets_s, targets_lt, targets_u, targets_lt, targets_u], dim=0)
        if args.alpha > 0:
